...
async function main() {
  // Deploy IPFSStorage contract first since it's needed by other contracts
  ...
  // Deploy MedChainUpgradeable implementation
  console.log("\nDeploying MedChainUpgradeable implementation...");
  const MedChainUpgradeable = await ethers.getContractFactory("MedChainUpgradeable");
  const medChainImpl = await MedChainUpgradeable.deploy();
  await medChainImpl.waitForDeployment();
  console.log("MedChainUpgradeable implementation deployed to:", await medChainImpl.getAddress());

  // Deploy MedChainFactory
  console.log("\nDeploying MedChainFactory...");
  const MedChainFactory = await ethers.getContractFactory("MedChainFactory");
  const medChainFactory = await MedChainFactory.deploy(await medChainImpl.getAddress());
  await medChainFactory.waitForDeployment();
  console.log("MedChainFactory deployed to:", await medChainFactory.getAddress());

  // Deploy a proxy instance through the factory
  console.log("\nDeploying MedChain proxy instance...");
  const initData = MedChainUpgradeable.interface.encodeFunctionData("initialize", [deployer.address]);
  const deployTx = await medChainFactory.deployMedChain(initData);
  const receipt = await deployTx.wait();
  const proxyAddress = receipt.logs[0].args.medChainAddress;
  console.log("MedChain proxy deployed to:", proxyAddress);

  // Create a contract instance for the proxy
  const medChain = await ethers.getContractAt("MedChainUpgradeable", proxyAddress);
